...
class Sqlite:
    ...
    def get_user_followers(self, username, limit=1000):
        """ Select user's followers """

        # execution time is not affected much with separate query to get id
    ...
    def get_user_following(self, username, limit=1000):
    ...
    def get_tweets_for_doc2vec(self, limit=10000):
        """ Returns all tweets and user id """
        result = self.cursor.execute(
        f"""
            SELECT t.author_id, u.username, u.name, t.text
            FROM tweets t
            JOIN users u
            ON u.id = t.author_id
            LIMIT {limit}
    ...
    def get_mutual_following(self, username):
        ...
        try:
            ...
            f"""
                ...
                VALUES (
                    ...
                    '{values[6]}', -- referenced_tweets_id
                    '{values[7]}', -- referenced_tweets_type
                    '{values[8]}', -- retweet_count
                    '{values[9]}', -- reply_count
                    '{values[10]}', -- like_count
                    '{values[11]}', -- quote_count
                    '{values[12]}', -- lang
                    '{values[13]}' -- source
                    -- + default sentiment
                    );
    ...
    def get_user_last_tweet_date(self, username):
        user = self.get_user_by_username(username)
        user_id = user[0]
        result = self.cursor.execute(
            f"""
    ...
    def get_user_timeline(self, username, limit=100):
        result = self.cursor.execute(
            f"""
                SELECT *
                FROM tweets
